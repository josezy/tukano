...
def detect_good_board_data(image, arucoParams, data_ammount,path,show_detections=False):

    name = path + str(data_ammount)+".jpg"
    img_gray = rgb2gray(image).astype(np.uint8)
    corners, ids, rejectedImgPoints = aruco.detectMarkers(img_gray, self.aruco_dict, parameters=arucoParams)

   
    if len(corners)!= self.horizontal_tag_ammount*self.vertical_tag_ammount :
        ...
        img_aruco = aruco.drawDetectedMarkers(image.copy(), corners, ids, (0,255,0))
        if show_detections:
...
def update_landing(drone,initialLocation,angle_x,angle_y,tvec,dist_to_marker=0,hertz=5):
    ...
    drone.mav.landing_target_send(
        ...
        angle_x,
        angle_y,
        dist_to_marker if dist_to_marker==0 else drone.location().alt - initialLocation.alt,  # distance drone-target
